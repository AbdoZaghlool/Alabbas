...
{
    if ($oldImage != 'default.png' && file_exists(public_path('/' . $folderNam . '/' . $oldImage))) {
...
function saveNotification($userId, $title, $type, $message, $id = null, $driver_id = null)
{
    $created = \App\Notification::create([
        'user_id' => $userId, 'title' => $title, 'type' => $type,
        'message' => $message, 'order_id' => $id, 'driver_id' => $driver_id
    ]);
    return $created;
}
